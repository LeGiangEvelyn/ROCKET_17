...
public class DepartmentSpecification implements Specification<Department> {
	...
	public Predicate toPredicate(Root<Department> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
		if (operator.equalsIgnoreCase("LIKE")) {
			if (field.equalsIgnoreCase("address.addressName")) {
				Join<Department, Address> joinDepartAddress = root.join("address", JoinType.LEFT);
				return builder.like(joinDepartAddress.get("addressName"), "%" + value.toString() + "%");
				}
